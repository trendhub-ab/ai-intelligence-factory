...
def test_pending_validation_reader_only_gets_exactly_one_repair():
    p = _pipeline()
    run284.install(p)
    assert p.should_attempt_dynamic_retry([_reader_row()], _safe_evidence(), "pending_retry_validation") == (
        True,
...
def test_pending_validation_rejects_hard_reader_blocker():
    ...
    assert p.should_attempt_dynamic_retry(
        [_reader_row(severity="HARD")], _safe_evidence(), "pending_retry_validation"
    )[0] is False
...
def test_pending_validation_rejects_unsafe_evidence():
    ...
    run284.install(p)
    assert p.should_attempt_dynamic_retry(
        [_reader_row()], {"state": "INSUFFICIENT", "decision_scope_safe": False}, "pending_retry_validation"
    )[0] is False
...
def test_existing_current_policy_lane_remains_authorized():
    ...
    run284.install(p)
    assert p.should_attempt_dynamic_retry(
        [_reader_row("non_engineer_access_failure")], _safe_evidence(), "current_policy_ready_recovery"
    ) == (True, "run284_current_policy_reader_repair")
...
def test_run373_unknown_or_fact_reason_cannot_create_reader_plan():
    rows = [
        {"severity": "REVIEW", "message": "fact_validation:unsupported_claim"},
        {"severity": "REVIEW", "message": "reader_value_review:unknown_future_label"},
    ]
    assert run284.deterministic_reader_repair_plan(rows) == ()
...
def test_run373_article_like_fixture_targets_run372_reader_failures_without_new_facts():
    # Article-like fixture: the content itself stays outside deterministic rewriting.
    # The zero-API layer only emits an edit contract from the observed Gate reasons.
    article = (
        "Agent MemoryはMCP、SQLite、embedding、namespace、retrieval policyを組み合わせて記憶を扱う。"
        "複数の実装要素があり、導入時には保存先、検索方式、クライアント連携を確認する必要がある。"
        "判断としては、複数ツールで同じ記憶を参照したい場合に検証価値がある。"
    )
    rows = [
        _reader_row("dense_report_cluster"),
        _reader_row("multi_axis_reader_weakness"),
    ...
    text = run284.reader_repair_feedback(rows)
    assert article  # fixture proves we do not mutate source text in this deterministic stage.
...
def test_run373_human_appeal_and_reader_repair_are_not_self_contradictory():
    text = run284.reader_repair_feedback([
        _reader_row("multi_axis_reader_weakness"),
        _reader_row("non_engineer_access_failure"),
    ])
    # No fabricated friendliness, but reader pull from existing facts remains explicitly allowed.
