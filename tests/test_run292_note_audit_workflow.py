...
class Run292NoteAuditWorkflowTests(unittest.TestCase):
    ...
    def test_live_audit_uses_run295_shared_eyecatch_proof_entrypoint(self) -> None:
        self.assertIn("xvfb-run -a python run295_note_private_draft_audit.py", self.source)
        ...
        self.assertIn("tests.test_run294_note_eyecatch_persistence_diagnostics", self.source)
        self.assertIn("tests.test_run295_note_eyecatch_persistence", self.source)
        self.assertIn("NOTE_AUDIT_PREPARE_ONLY: 'false'", self.source)

    def test_run295_contract_runs_before_live_browser_audit(self) -> None:
        live = self.source.split("  audit-private-draft:", 1)[1].split("\n  stop-cloud-vm:", 1)[0]
        test_pos = live.find("python -m unittest tests.test_run295_note_eyecatch_persistence -v")
        audit_pos = live.find("xvfb-run -a python run295_note_private_draft_audit.py")
        self.assertGreaterEqual(test_pos, 0)
        self.assertGreater(audit_pos, test_pos)
        self.assertIn("Run291/292/293/294/295 read-only regression tests", live)
    ...
    def test_safe_summary_runs_even_when_live_audit_fails_closed(self) -> None:
        ...
        self.assertIn("large_top_img_count", section)
        self.assertIn("large_top_background_count", section)
        self.assertIn("eyecatch_exact_control_visible_count", section)
        self.assertIn("eyecatch_proof_mode", section)
        self.assertIn("Run295 private note draft read-only audit", section)
        self.assertIn("unpublished content / draft URL / image URL exposed: `false`", section)
