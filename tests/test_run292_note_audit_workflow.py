...
class Run292NoteAuditWorkflowTests(unittest.TestCase):
    ...
    def test_live_audit_uses_run294_eyecatch_diagnostic_entrypoint(self) -> None:
        self.assertIn("xvfb-run -a python run294_note_eyecatch_persistence_diagnostics.py", self.source)
    ...
    def test_run294_diagnostic_contract_runs_before_live_browser_audit(self) -> None:
        live = self.source.split("  audit-private-draft:", 1)[1].split("\n  stop-cloud-vm:", 1)[0]
        test_pos = live.find("python -m unittest tests.test_run294_note_eyecatch_persistence_diagnostics -v")
        audit_pos = live.find("xvfb-run -a python run294_note_eyecatch_persistence_diagnostics.py")
        self.assertGreaterEqual(test_pos, 0)
        self.assertGreater(audit_pos, test_pos)
        self.assertIn("Run291/292/293/294 read-only regression tests", live)
    ...
    def test_safe_summary_runs_even_when_live_audit_fails_closed(self) -> None:
        ...
        self.assertIn("Run294 private note draft read-only audit", section)
