...
def test_second_experiment_v2_measures_one_of_five_without_provider_or_gate_changes():
    code = r"""
import importlib.util
import json
import re
...
assert summary["pass_count"] == 1, "V2_RESULT::" + json.dumps(summary, ensure_ascii=False, sort_keys=True)
"""
    env = dict(
        os.environ,
        SYNTHETIC_REGRESSION_MODE="true",
...
def test_second_experiment_v3_general_rules_reach_five_of_five_without_provider_or_gate_changes():
    code = r"""
import importlib.util
import json
import re
...
socket.socket.connect = lambda *a, **k: (_ for _ in ()).throw(
    AssertionError("network forbidden in second Local Writer v3 experiment")
)

root = Path.cwd()
exp2 = root / "experiments" / "local_writer_five_article_20260926"

spec = importlib.util.spec_from_file_location("local_writer_v3", exp2 / "local_writer_v3.py")
writer = importlib.util.module_from_spec(spec)
spec.loader.exec_module(writer)
batch = json.loads((exp2 / "snapshots.json").read_text(encoding="utf-8"))
...
assert summary["pass_count"] == 5, "V3_RESULT::" + json.dumps(summary, ensure_ascii=False, sort_keys=True)
"""
    env = dict(
        os.environ,
        SYNTHETIC_REGRESSION_MODE="true",
