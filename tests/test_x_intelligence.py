...
def test_build_x_post_is_zero_api_and_review_only():
    draft = build_x_post(_item())

    assert draft["status"] == "X Pending Review"
    assert draft["gemini_calls"] == 0
    assert draft["x_api_calls"] == 0
    assert draft["auto_posted"] is False
    assert draft["generator_mode"] == "deterministic_zero_api"


def test_build_x_post_retains_primary_source_and_stays_within_limit():
    item = _item(
        **{
    ...
    assert item["URL"] in draft["post"]
    assert "一次情報：" in draft["post"]


def test_build_x_post_fails_closed_without_source_url():
    with pytest.raises(ValueError, match="primary source URL"):
        build_x_post(_item(URL=""))


def test_selector_accepts_strong_decision_or_screening_signal():
    items = [
...
def test_selector_does_not_mutate_input():
    ...
    assert "x_candidate_score" in selected[0]
    assert "x_primary_url" in selected[0]


def test_selector_is_deterministic_and_ranked():
    ...
    assert [x["Name"] for x in first] == ["high", "low"]
    assert first == second


def test_save_pending_post_writes_only_review_artifacts(tmp_path: Path):
    draft = build_x_post(_item())
    json_path, md_path = save_pending_post(draft, output_dir=tmp_path, stem="sample")

    assert json_path == tmp_path / "sample.json"
    assert md_path == tmp_path / "sample.md"
    assert json_path.exists()
    assert md_path.exists()
    assert "X Pending Review" in md_path.read_text(encoding="utf-8")
    assert '"x_api_calls": 0' in json_path.read_text(encoding="utf-8")
