...
class Run131ReaderProximityInformationBudgetReconciliationTests(unittest.TestCase):
...
一次資料の制約を確認して比較します。"""
    ...
    def test_information_budget_flags_dense_double_explanation_only_softly(self):
        dense = (
            "Transformer Attention Routing Adapter Cache Distillation Gradient Token Policy Model "
            "アテンションルーティングキャッシュアダプタモデルを説明します。"
        )
        article = "\n\n".join([
            dense + " たとえば身近な箱のようなものです。" for _ in range(4)
        ])
        sig = pipeline._reader_experience_signals(article)
        self.assertEqual("REVIEW", sig["information_budget"])
        self.assertTrue(sig["soft_only"])

    def test_information_budget_does_not_become_a_hard_gate(self):
        source = Path(pipeline.__file__).read_text(encoding="utf-8")
        self.assertNotIn('"information_budget" in hard', source)
        self.assertNotIn("information_budget_failure", source)
        self.assertNotIn("reader_proximity_missing\"):\n        return False", source)

    def test_everyday_scene_alone_cannot_fake_reader_proximity(self):
        article = """朝、スマホを開きます。買い物や通勤でもAIは身近です。アクセス制御は権限範囲を限定する仕組みです。
...
一次資料の制約を確認して比較します。"""
    ...
    def test_repeated_reader_questions_are_not_rewarded(self):
        article = " ".join(["使ったことはありませんか。" for _ in range(7)])
        sig = pipeline._reader_experience_signals(article)
        self.assertTrue(sig["conversational_overuse"])
        self.assertEqual("REVIEW_OVERUSE", sig["reader_proximity"])
