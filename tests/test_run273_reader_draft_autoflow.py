...
class Run273PrivateDraftAutoflowTests(unittest.TestCase):
    def test_ready_sync_manual_route_can_chain_to_private_draft(self) -> None:
        ...
        self.assertIn("Resolve exact private-draft fan-out eligibility", source)
        self.assertIn("steps.fanout.outputs.eligible == 'true'", source)
    ...
    def test_push_policy_reconciliation_cannot_enter_private_draft_dispatch_step(self) -> None:
        ...
        dispatch_condition = source.index("if: ${{ github.event_name == 'workflow_dispatch'", dispatch_step)
        command = source.index("gh workflow run note-create-draft.yml", dispatch_condition)
        self.assertLess(preflight_step, preflight_condition)
        self.assertLess(preflight_condition, dispatch_step)
        self.assertLess(dispatch_step, dispatch_condition)
        self.assertLess(dispatch_condition, command)
