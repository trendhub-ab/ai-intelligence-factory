...
CURRENT_DATA_SOURCE_ID = "7e4ceaa7-7bdf-4c4b-bf78-c2cccac44404"
PRE_RUN220_DB_ID = "d6ca3c1f-cb2c-4686-b442-d9ba3923e5f1"
PRE_RUN220_DATA_SOURCE_ID = "d1461b6f-0940-4bf9-803a-6686a37c4ba2"
...
class Run217MemberCommerceContractTests(unittest.TestCase):
    ...
    def test_run217_remains_commerce_history_but_later_authorities_are_explicit(self):
        ...
        self.assertIn("DB destination superseded by Run220", self.run217)

    def test_current_canonical_home_is_locked(self):
        for text in (self.readme, self.spec, self.run217, self.run218, self.run220):
            self.assertIn(CANONICAL_MEMBER_HOME_ID, text)
            self.assertIn("AI Decision Intelligence｜会員ホーム", text)

    def test_current_db_ids_are_run220_ids(self):
    ...
    def test_pre_run220_database_is_explicitly_quarantined(self):
        for value in (PRE_RUN220_DB_ID, PRE_RUN220_DATA_SOURCE_ID):
            self.assertIn(value, self.readme)
            self.assertIn(value, self.spec)
            self.assertIn(value, self.run220)
        self.assertIn("旧版・使用禁止", self.run220)

    def test_run217_created_home_is_explicitly_superseded(self):
    ...
    def test_legacy_100_row_database_is_forbidden(self):
    ...
    def test_member_product_corrections_remain_zero_model_and_daily_paused(self):
