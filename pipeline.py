...
from reader_experience_signals import reader_experience_signals as _reader_experience_signals_impl
from editorial_naturalness import (
...
import fact_validation_signals as _fact_validation_signals_module
...
def _bind_run245_fact_runtime() -> None:
...
def _normalize_numeric_evidence_text(text: str):
    _bind_run245_fact_runtime()
    return _normalize_numeric_evidence_text_impl(text)


def _numeric_claim_condition_tags(text: str):
    _bind_run245_fact_runtime()
    return _numeric_claim_condition_tags_impl(text)


def _numeric_condition_compatible(claim_window: str, evidence_window: str):
    _bind_run245_fact_runtime()
    return _numeric_condition_compatible_impl(claim_window, evidence_window)


def _is_protocol_cardinality_expression(text: str, start: int, end: int, token: str):
    _bind_run245_fact_runtime()
    return _is_protocol_cardinality_expression_impl(text, start, end, token)


def _find_unsupported_numeric_claims(draft: str, source_context: str, evidence_metadata: dict | None = None):
    _bind_run245_fact_runtime()
...
def _claim_is_negated(text: str, start: int, end: int):
    _bind_run245_fact_runtime()
    return _claim_is_negated_impl(text, start, end)


def _find_hype_claims(draft: str, source_context: str = "", evidence_metadata: dict | None = None):
    _bind_run245_fact_runtime()
    return _find_hype_claims_impl(draft, source_context, evidence_metadata)


def _evidence_has_substantive_coverage(key: str, source_context: str, evidence_metadata: dict | None = None):
    _bind_run245_fact_runtime()
    return _evidence_has_substantive_coverage_impl(key, source_context, evidence_metadata)


def _find_false_negative_evidence_claims(draft: str, evidence_metadata: dict, source_context: str = ""):
    _bind_run245_fact_runtime()
    return _find_false_negative_evidence_claims_impl(draft, evidence_metadata, source_context)
...
def _find_unsupported_competitor_claims(parsed: dict, source_context: str):
    _bind_run245_fact_runtime()
    return _find_unsupported_competitor_claims_impl(parsed, source_context)
...
def _expand_evidence_aliases(source_context: str):
    _bind_run245_boundary_runtime()
    return _expand_evidence_aliases_impl(source_context)


def _find_source_boundary_violations(draft: str, source_context: str, repo_name: str = ""):
    _bind_run245_boundary_runtime()
    return _find_source_boundary_violations_impl(draft, source_context, repo_name)
...
def _relation_family_for_predicate(predicate: str):
    _bind_run245_fact_runtime()
    return _relation_family_for_predicate_impl(predicate)


def _clean_relation_entity(value: str):
    _bind_run245_fact_runtime()
    return _clean_relation_entity_impl(value)


def _looks_like_relation_entity(value: str):
    _bind_run245_fact_runtime()
    return _looks_like_relation_entity_impl(value)


def _extract_explicit_relation_claim(sentence: str):
    _bind_run245_fact_runtime()
    return _extract_explicit_relation_claim_impl(sentence)


def _evidence_supports_relation(actor: str, obj: str, family_patterns: tuple[str, ...], source_context: str):
    _bind_run245_fact_runtime()
    return _evidence_supports_relation_impl(actor, obj, family_patterns, source_context)


def _find_entity_relation_violations(draft: str, source_context: str):
    _bind_run245_fact_runtime()
    return _find_entity_relation_violations_impl(draft, source_context)
...
def validate_fact_gate(parsed: dict, repo_name: str, source_context: str = "", source: str = "",
    ...
    failures.extend(_find_causal_inference_overclaims(draft, source_context))
...
_promote_plaintext_section_titles = _promote_plaintext_section_titles_impl
