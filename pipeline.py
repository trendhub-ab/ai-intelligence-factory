...
・Reader Proximityは「使ってもよい装飾」ではなく、無料note記事の完成条件として扱う。ただし品質Gateを緩めたり、親しみ不足だけを理由にGemini再生成を増やしたりしない。記事全体の温度を1〜2個の口語句で済ませず、硬い説明が2段落続いたら次の段落では、追加説明を足さず、既存文を「読者の判断／具体場面／平易な一言」のどれかへ置き換えて人間の言葉へ戻す。ARTICLE全体が長い場合は段落追加ではなく削除・統合を優先する。
...
・最終出力前にARTICLEだけを読者目線で再編集する。目標は2,200〜3,000字、3,200字はSoft Ceiling。ただし3,200字を超えそうなら、Evidence・数値・制約・比較・反証・Decisionを残し、実装手順の網羅、固有技術名の列挙、二重説明、長いコード例、一般論を先に削って完成させる。『詳しく書けるから書く』は禁止とする。3,200字を超えそうなら新しい説明を足さず、Decisionに不要な技術詳細を圧縮する。どうしても重要Evidenceや制約のため超える場合は許容するが、4,000字級を『専門テーマだから仕方ない』で正当化しない。
...
def build_dynamic_retry_instruction(reason_rows: list[dict]) -> tuple[str, list[str]]:
    ...
    if any((row.get("reason_code") or "") in {REASON_CODE_APPEAL_AI_STYLE_COMPOSITE, REASON_CODE_APPEAL_CROSS_ARTICLE_FINGERPRINT} for row in reason_rows):
        ...
        instructions.append("修正対象外の一次情報・数値・固有名詞は不用意に書き換えないでください。ただしARTICLEが3,200字を超えている場合は、局所修正だけで長文を温存せず、Evidence・数値・制約・比較・反証・Decisionを保持したまま、実装列挙・二重説明・一般論・長いコード例を削除または統合して2,200〜3,000字へ再編集してください。Retryで本文を長くすることは禁止です。根拠にない保証表現、業界標準との断定、時間・金額・性能などの数値を新たに補わないでください。")
