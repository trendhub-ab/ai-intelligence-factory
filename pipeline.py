    ...
    for model_name in candidates:
        ...
        for attempt in range(PING_MAX_RETRIES + 1):
            try:
                ...
                last_error = e
                logger.exception("[MODEL RESOLUTION ERROR] model=%s attempt=%s", model_name, attempt + 1)
                raise NoAvailableModelError(f"モデル初期化失敗: {type(e).__name__}: {e}") from e
...
def call_gemini_grounded_deep_dive(prompt: str, repo: dict, source_info: dict,
    ...
    for attempt in range(2):
        ...
        try:
            ...
            DEEP_DIVE_MODEL_BUDGET.consume(kind)
            time.sleep(3)
            _consume_gemini_request(SELECTED_DEEP_DIVE_MODEL, kind)
