...
def parsed(core):
    return {"title_text":FINAL_TITLE,"note_draft":core,"action_text":"割引ルールを1つ検証環境へ移し、同じ入力で既存実装との出力一致を比較検証し、変更作業と実行時間を計測する。","decision_text":"WATCH","score":55,"decision_reason_text":"ルール分離の価値はあるが、本番導入前に互換性・運用・性能を小さく検証すべき段階。","source_summary_text":"GoRulesは業務判断ルールを独立して管理・実行する仕組みで、中核エンジンはオープンソース。","what_text":"業務上の判断ルールをコード本体から切り分けて管理・実行する仕組み。","why_important_text":"変更頻度の高い判断条件を独立管理する選択肢になる。"}
