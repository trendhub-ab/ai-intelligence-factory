...
Existing Notion copy is treated as the quality reference. No migration may rewrite existing
values. Rich editorial comments stay production-frozen unless a separate shadow candidate
proves semantic and presentation parity. Only score breakdown is currently eligible for a
deterministic shadow because it is a pure formatting projection of locked numeric scores.
"""
...
# Golden-sample audit of completed Content DB entries showed that rich fields contain
# concrete implementation/PoC guidance which a categorical Plan cannot reproduce without
# material information loss. Therefore only the numeric score breakdown remains shadowable.
CONTENT_COMMENT_CONTRACT = {
    ...
    "なぜ重要？": (LEGACY_ARTICLE_MANAGEMENT_FROZEN, PRODUCTION_FROZEN),
    "なぜ重要ではない？": (LEGACY_ARTICLE_MANAGEMENT_FROZEN, PRODUCTION_FROZEN),
    "対象": (LEGACY_ARTICLE_MANAGEMENT_FROZEN, PRODUCTION_FROZEN),
    "次にやること": (LEGACY_ARTICLE_MANAGEMENT_FROZEN, PRODUCTION_FROZEN),
    ...
    "判断理由": (LEGACY_ARTICLE_MANAGEMENT_FROZEN, PRODUCTION_FROZEN),
...
def _score_breakdown(plan: Mapping) -> str:
    total = sum(int(plan[key]) for key in (
        "business_impact", "technical_impact", "urgency", "market_impact", "reliability"
    ))
    # Preserve the established Content DB presentation exactly: semicolon-delimited axes + total.
    return (
        ...
        f"Reliability {plan['reliability']}/15; 合計 {total}/100"
    )


def build_content_comment_shadow(plan_report_path: str) -> dict:
    plan = _strict_fact_locked_plan(plan_report_path)
    values = {"スコア内訳": _score_breakdown(plan)}
    shadowable = {name for name, (_, state) in CONTENT_COMMENT_CONTRACT.items() if state == SHADOW_ONLY}
    if set(values) != shadowable:
