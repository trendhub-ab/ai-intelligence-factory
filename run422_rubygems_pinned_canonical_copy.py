#!/usr/bin/env python3
"""Run422: exact RubyGems canonical eyecatch with pinned source-bounded copy.

Run421 proved that the generic 48-char hook can still fail the real Run180/Noto contract.
For this exact already-approved article, use a short deterministic copy that preserves the
protected OpenAI/RubyGems identifiers and the article's existing permissions-management
subject. No model request is used. All pixels are rendered by the installed Production
visual stack inherited from Run418/419/421.
"""
...
def _pinned_plan() -> dict[str, Any]:
    ...
    if r180._validate_eyecatch_title(source_title, PINNED_EYECATCH_TITLE) != PINNED_EYECATCH_TITLE:
        raise r418.Run418Error("Run422 pinned eyecatch title failed Run180 semantic guard")
...
def _render_zero_model():
    ...
    if str(result) != str(r418.OUTPUT) or not r418.OUTPUT.exists() or r418.OUTPUT.stat().st_size < 10_000:
        raise r418.Run418Error("Run422 canonical eyecatch render missing or unexpectedly small")
    with Image.open(r418.OUTPUT) as image:
        if image.size != (1280, 670) or image.format != "PNG":
            raise r418.Run418Error(f"Run422 canonical eyecatch geometry invalid: {image.size} {image.format}")
...
def main() -> int:
    ...
    if r418.os.getenv("RUN418_CONFIRM", "").strip() != r418.CONFIRM:
        raise r418.Run418Error("Run422 exact-target confirmation missing")
