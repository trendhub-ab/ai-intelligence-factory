...
repair prompt now translates concrete gate reasons into an executable deletion budget
instead of merely repeating the general Reader principles.
...
2. 制約保持：重要な制約・対象範囲・例外・未検証条件は削らず、普通の日本語で1〜2文に圧縮してDecisionの近くに残す。
3. Evidence保持：Decisionを支える一次情報・重要数値・反証は残す。ただしEvidenceの深さを技術名の多さで表現しない。
4. 中核メカニズム：読者が「なぜそうなるか」を理解するための仕組みは原則1つを主役にする。
5. 実装名・略語・ベンチマーク名：Decisionも制約も変えない名前は本文から外すか、意味カテゴリへ圧縮する。

・読者が前半で覚える中心メッセージは原則3つまで。①変化 ②判断 ③判断を変えうる重要な制約を優先する。
・冒頭約600文字では中核メカニズムを1つまでに絞り、Decisionに不要なAPI名・内部構造・精度名・ベンチマーク条件・実装識別子を並べない。
・専門語を説明するために別の未説明専門語を持ち込まない。最初の専門語・略語は同じEvidenceの範囲で一度だけ普通の日本語に言い換える。
・問いかけや比喩は、それだけではReader Bridgeとみなさない。「自分に関係する理由」「判断の速さ」「具体的な次Action」を優先する。
・高密度な技術説明を2段落続けない。技術説明の次は新しい技術名を足さず、その事実が読者の判断・制約・行動のどれを変えるかへ戻る。
・方法名、略語、ベンチマーク、内部部品が3個以上並びそうならEvidence inventoryとみなし本文から退避する。例外は個々の名前の違い自体がDecisionを変える場合だけ。
・本文後半でも新しい専門概念を次々追加しない。後半は前半のDecisionをEvidence・条件・比較・次Actionで精密化する。
...
・前稿の後半に既に存在するDecision/Actionは意味を変えずに冒頭3段落以内へ前倒ししてよい。
・前稿にある重要な制約・対象範囲・例外・未検証条件は削除せず、意味を保った普通の日本語へ置換する。
・冒頭約600文字では中核メカニズムを1つまでにする。Decisionに不要な専門語・実装識別子は後段へ移すだけでなく、まず削除・カテゴリ化する。
・方法名、略語、ベンチマーク、内部部品の列挙は、各名称がDecisionを変えない限り「複数の既存手法」等へ圧縮する。
・記事全体で新規概念を増やさず、既存Evidenceを「何を意味するか → なぜ判断に効くか → どんな制約があるか」の順へ並べ替える。
・修正後も事実Gate、Evidence Gate、Publication Gate、Reader Gateをすべて再判定し、通らなければReadyにしない。
""".strip()

# Run359: executable repair directives. These are intentionally reason-triggered. They do
# not impose a universal word-count or acronym quota on healthy articles.
...
def _fresh_evidence_safe(pipeline_module: Any, evidence_result: dict | None) -> bool:
    ...
    return str(evidence_result.get("state") or "") == expected and evidence_result.get("decision_scope_safe") is True
...
def install(pipeline_module: Any) -> Any:
    ...
    def should_attempt_dynamic_retry_with_reader_repair(reason_rows: list[dict], evidence_result: dict | None, candidate_origin: str = "new"):
        ...
        if (
            os.getenv(FAST_LANE_ENV, "") == "1" and candidate_origin == "pending_retry"
            and evidence_result is not None and not getattr(pipeline_module, _SPENT_ATTR, False)
