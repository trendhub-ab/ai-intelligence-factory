"""Phase 0.5 batch runner for the isolated X Intelligence Layer.

The runner consumes already-produced Factory records from JSON, JSONL, or CSV,
selects suitable candidates, and writes review-only X draft artifacts.
...
def load_records(path: str | Path) -> list[dict[str, Any]]:
    """Load Factory-like records from JSON, JSONL/NDJSON, or CSV."""
...
def generate_batch(
    ...
    *,
    ...
    """Select candidates and create human-review artifacts plus a manifest."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Generate review-only X drafts from Factory exports")
