...
The pinned title must first pass `Run180._validate_eyecatch_title` against the original note title. Typography is then rebuilt with the Run419 deterministic repair helper and the final plan must pass the full Run180 geometry contract.
...
`tests/test_run422_pinned_canonical_copy.py` locks the required source identifiers, bounded copy length, zero-model/no-raw-renderer path, exact-target mutation boundary and no-publication contract.
