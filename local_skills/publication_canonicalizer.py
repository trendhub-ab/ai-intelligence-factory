...
def canonicalize_snapshot(snapshot: Mapping[str, Any]) -> dict[str, Any]:
    # Idempotency is part of the frozen v4 contract. A record already compiled
    # by this exact version must not accumulate glossary/scope annotations.
    if snapshot.get("publication_canonicalizer_version") == "stage6-v4":
