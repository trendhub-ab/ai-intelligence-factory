...
replacements = [
    (
        ...
        "2,200〜3,000字を実務上の完成レンジとし、3,200字はSoft Ceiling。3,200字を超えそうなら、Evidence・数値・制約・比較・反証・Decisionを残し、実装手順の網羅、固有技術名の列挙、二重説明、長いコード例、一般論を先に削って完成させる。『詳しく書けるから書く』は禁止"
    ),
    (
        "一次情報に存在する技術名を全部ARTICLEへ転記することは禁止",
        "一次情報に存在する技術名を全部ARTICLEへ転記することは禁止。ARTICLE本文で説明する中核概念は原則2〜3個、実装識別子・規格名・コマンド名は意思決定に必要なものだけに限定し、列挙で専門性を演出しない"
    ),
    (
        "硬い説明が2段落続いたら次の段落",
        "硬い説明が2段落続いたら次の段落は、追加説明を足すのではなく、既存説明を読者の判断・具体場面・平易な一文へ置き換える。ARTICLE全体が長い場合は段落追加ではなく削除・統合を優先する"
    ),
    (
