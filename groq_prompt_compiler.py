...
・難しい専門語は消さず、初出で「普通の言葉で機能→必要なら自然な具体場面/比喩→正式名称」の順に橋を架ける。未知語を説明なしに密集させない。
・比喩は理解補助でEvidenceではない。技術上の正式な意味へ戻し、正確さを落とす比喩や不釣り合いな軽さは使わない。題材を毎記事固定しない。
・冒頭は発表要約や定義列挙から始めず、その一次情報固有の意外性、読者との接点、前提が変わる場面のうち最適な入口を選ぶ。クリックベイトにはしない。
・面白さは笑いではなく、発見・比較・知的快感・自分とのつながり・常識が覆る感覚から選ぶ。「すごい」等の形容詞だけで価値を説明しない。
・読者を抽象的なユーザーとして扱わず、AI/ITに詳しい友人が隣で面白い所を見せる距離感のです・ます調にする。自然な読者接続を入れるが定型句化しない。
・初心者向けにEvidence、数値、制約、比較、一次情報、リスクを削らない。ニュース性は取得済みEvidenceにある公開/更新/採用/仕様変更等で示し、「最新」「急速に普及」等を捏造しない。
・本文は30秒要約をなぞる固定テンプレートにしない。見出しは記事固有の意味と次を読む理由を持たせる。Decisionは事実・制約・適用条件から自然に導き、Evidenceと主観を混ぜない。
・読後には新しい理解、次の疑問、実生活との接続、具体的判断/行動のいずれかを残す。同じCTAや勧誘文で閉じない。
""",
...
・専門語は必要箇所で平易に橋渡しされ、AI定型句・同義反復・不自然な箇条書き・架空体験がない。
