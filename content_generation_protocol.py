...
・一つの説明段落に複数の新しい専門概念・実装部品・条件を詰め込まない。段落の仕事を「仕組み」「限界」「読者への意味」など
  一つに定め、別の仕事は次の段落へ送る。これは固定文字数・固定個数ルールではなく、理解順序のルールである。
・一次情報に実装名が多数あっても、Decisionを変えない部品名・内部識別子・規格列挙はARTICLEへ運ばない。
  Evidence・重要な数値・能力境界・反証・Decisionは残し、重複説明と実装在庫だけを圧縮する。
・難所ではクレハの疑問を「用語の意味」だけで終わらせず、「それで使う側には何が変わる？」「できないことは何？」のように
  Reader Decisionへ接続する。フェルンの返答は新しい情報を足さず、直前のEvidenceを平易に翻訳してから正式な条件へ戻す。
・掛け合いは説明の追加レイヤーにしない。同じ内容を本文と会話で二重説明する場合は、会話を削るか本文を置換する。
  会話を増やしてHuman Appealを作るのではなく、理解の詰まりを会話でほどく。
・各主要セクションを終える時、非エンジニアが「ここまでで何が分かったか」を追える流れにする。
  ただし毎節の要約文をノルマ化せず、次の疑問・制約・判断へ自然につなぐ。
・タイトルは技術名だけ、煽りだけ、閉じ括弧だけが残る形にしない。「何が変わる話か」が日本語として一読で取れる表面にする。
・出力直前の内部セルフチェックでは、Accessibility / Information Budget / Jargon Translation /
  Non-Engineer Core Clarity / Narrative Pull / Reader Temperature-Rhythmを想定し、弱い箇所は新しいFactを足さずに
  削除・分割・平易化・順序変更で一度だけ直す。Reader Valueだけの追加Provider Retryは要求しない。

【Single Reader Spine｜レポートを記事へ変える】
・ARTICLEは「Central Conclusion → なぜ今それが意外か → 仕組みを理解する最小限 → できないこと／条件 → 読者の判断」という一つの読解線で組む。一次情報の章立てや部品一覧をそのまま記事の骨格にしない。
・専門概念は普通語を先、正式名称を後に置く。正式名称を知らなくても段落の意味が通る文を先に作り、必要な場合だけ括弧等で名称へ戻す。
・フェルンとクレハの会話は説明の追記ではなく、硬い説明を会話で置換する。同じEvidenceを本文で説明してから会話でも繰り返さない。
・Decisionを変えない実装名、略語、内部部品、ベンチマーク識別子は本文へ持ち込まない。一次情報にあること自体は掲載理由にならない。
・主要段落は「一つの主張、一つの意味、一つの制約」を基本単位にする。複数の新概念を一段落へ積み上げて技術レポート化しない。
・読者が技術名を覚えることではなく、「何が変わる／何はまだ変わらない／自分なら何を確認する」を理解することをARTICLEの情報予算にする。

【Canonical Reader Pattern｜Acceptedを「また読みたい」へ引き上げる】
・仕組み説明で本文へ残す実装概念は、読者のDecisionを実際に変える中核1〜2個を優先する。3個以上の部品名・略語・デコーダー等を並べる必要がある場合は、それぞれがDecisionを変える理由を確認し、変えないものは削るか普通語へ統合する。
・Decision / Action Spineを一本にする。本文中の「試す・導入する・比較する・待つ・注視する」等の行動提案は、最終Decisionと矛盾させない。条件付きPoCを提案するなら、その条件を最終Decisionにも残す。
・Evidenceの強さと文章の断定強度を揃える。「間違いない」「確実」「必ず」「決定的」「標準になる」等は一次情報が同じ強度で直接支える場合だけ使う。見通しや筆者判断なら条件・不確実性を明示するか、断定を弱める。
・フェルンとクレハは説明の挿入ではなく「違和感 → 検証 → 判断」を進める。クレハの疑問で読者の短絡を表面化し、フェルンはEvidenceの境界へ戻し、その結果として次の判断へ進む。会話を削っても本文が同じ順序・同じ意味で成立するなら、その会話は装飾なので削るか本文を置換する。
...
・タイトルと導入は本文より強い主張をしない。一次情報の条件を落として煽りを作らない。
