#!/usr/bin/env python3
"""Run364: re-sign only Run362-proven historical Ready manuscripts without changing body bytes.

The allowlist is frozen from the successful Run362 audit artifact. Every candidate must still
match its page id, title, source, primary URL, historical Ready policy SHA, historical main
commit and manuscript body SHA at execution time. Any drift fails closed for that row.

Dry-run performs no writes. Apply appends the exact existing manuscript body with the current
Ready caption, then reads it back and verifies byte identity. No model/provider calls and no
public note release are performed here.
"""
...
APPLY_CONFIRMATION = "REBASE_RUN362_PROVEN_8"
REPORT_PATH = Path("gate_history/run364_historical_ready_rebase_report.json")
...
ALLOWLIST: dict[str, dict[str, str]] = {
    ...
    "3cf479ff-dca9-81c7-b087-f6ab02d10a85": {
        "title": "AIエージェントに振り回されないためのスキル集「mattpocock/skills」とは？",
        "source": "HackerNews",
        "primary_url": "https://github.com/mattpocock/skills",
        "manuscript_sha256": "e4c77958659cbeceda4c0a6d994d48f903ff1662c19292b79b43e5f5719e56de",
        "historical_policy_sha256": "abc17c16c6ddca2eb2fcb0d82a6559482384740794338ff14bbda0f1124b9436",
        "historical_commit": "770c8fe89b35cb50b889237114b9e8dcf81769e7",
    },
...
def _verify_allowlist_history() -> None:
    if len(ALLOWLIST) != 8:
        raise RuntimeError(f"Run364 allowlist must contain exactly 8 rows, got {len(ALLOWLIST)}")
...
def classify_page(blocks: list[dict], expected: dict[str, str]) -> Decision:
    ready_family = []
    for index, block in enumerate(blocks):
        ...
        body_sha = contract.manuscript_sha256(body) if body else ""
        ready_family.append((index, body, fields, body_sha))

    current = [r for r in ready_family if r[1] and contract.is_current_ready_block(r[1], sync._code_caption(blocks[r[0]]))]
    if current:
        if len(current) != 1 or current[-1][3] != expected["manuscript_sha256"]:
            return Decision("blocked", "unexpected_current_ready_variant")
        return Decision("current", "already_current", manuscript_sha256=current[-1][3])

    matches = [
        r for r in ready_family
        if r[1]
        and r[2].get("policy_sha256") == expected["historical_policy_sha256"]
        and r[2].get("manuscript_sha256") == expected["manuscript_sha256"]
        and r[3] == expected["manuscript_sha256"]
    ]
    if len(matches) != 1:
        return Decision("blocked", f"exact_historical_match_count={len(matches)}")
    selected = matches[0]
    if selected[0] != ready_family[-1][0]:
        return Decision("blocked", "historical_match_not_latest_ready_family")
    return Decision(
        "eligible", "run362_proven_exact_body_and_policy", body=selected[1],
        manuscript_sha256=selected[3], historical_policy_sha256=expected["historical_policy_sha256"],
    )


def _current_code_block(body: str) -> dict[str, Any]:
    pieces = [body[i:i + RICH_TEXT_LIMIT] for i in range(0, len(body), RICH_TEXT_LIMIT)]
    if not pieces or "".join(pieces) != body:
        raise RuntimeError("segmentation changed manuscript bytes")
    return {
        "object": "block", "type": "code",
        "code": {
            "rich_text": [{"type": "text", "text": {"content": p}} for p in pieces],
            "language": "markdown",
            "caption": [{"type": "text", "text": {"content": contract.current_ready_caption(body)}}],
        },
    }
...
def run(mode: str = "dry_run", confirmation: str = "") -> dict[str, Any]:
    ...
    eligible: list[tuple[str, str, Decision]] = []
    for page_id, expected in ALLOWLIST.items():
        ...
        if page.status_code != 200:
            ...
            continue
        payload = page.json()
        state = sync._source_state(payload)
        if not state:
        ...
        if not state.get("eyecatch_url"):
            ...
            continue
        decision = classify_page(sync._block_children(page_id), expected)
        rows.append({
            "page_id": page_id, **pinned,
    ...
    if mode == "apply" and not blocked:
        if len(eligible) not in {0, 8}:
            raise RuntimeError(f"refusing partial Run364 apply: eligible={len(eligible)}")
        for page_id, title, decision in eligible:
            before = decision.body
            block = _current_code_block(before)
            if sync._code_body(block) != before:
                raise RuntimeError(f"local byte round-trip failed for {title}")
            response = sync._request("PATCH", f"https://api.notion.com/v1/blocks/{page_id}/children", json={"children": [block]})
            if response.status_code != 200:
                raise RuntimeError(f"Notion append failed for {title}: HTTP {response.status_code} {response.text[:250]}")
            after = sync._source_current_ready_manuscript(page_id)
            if after != before or contract.manuscript_sha256(after) != decision.manuscript_sha256:
                raise RuntimeError(f"post-write byte verification failed for {title}")
            applied += 1

    result = {
        "run": "run364_historical_ready_rebase",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "mode": mode,
        "allowlist_size": len(ALLOWLIST),
        "eligible": len([r for r in rows if r["status"] == "eligible"]),
        "already_current": len([r for r in rows if r["status"] == "current"]),
        "blocked": len(blocked),
        "applied": applied,
...
def main() -> int:
    result = run(
        mode=os.environ.get("AIIF_RUN364_MODE", "dry_run"),
        confirmation=os.environ.get("AIIF_RUN364_CONFIRM", ""),
    )
