...
def page_to_values(page: dict) -> dict[str, Any]:
    ...
    return {
        ...
        "name": _plain_text(p.get("Technology / Project Name")),
        "display_label": _plain_text(p.get("Japanese Display Label")),
        "category": _select(p.get("Category")),
        "adoption_score": _number(p.get("Adoption Score")),
        "adoption_status": _select(p.get("Adoption Status")),
        "production_readiness": _select(p.get("Production Readiness")),
        "evidence_confidence": _select(p.get("Evidence Confidence")),
        "plain_summary": _plain_text(p.get("Plain Summary")),
        "topic_trigger": _plain_text(p.get("Topic Trigger")),
        "short_rationale": _plain_text(p.get("Short Rationale")),
        "best_for": _plain_text(p.get("Best For")),
        "avoid_for": _plain_text(p.get("Avoid For")),
        "main_risk": _plain_text(p.get("Main Risk")),
        "primary_url": _url(p.get("Primary URL")),
        "evidence_urls": _split_urls(_plain_text(p.get("Primary Evidence URLs")), _url(p.get("Primary URL"))),
