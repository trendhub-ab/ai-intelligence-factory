...
def _validate_runtime_configuration() -> None:
    """検証でも重複確認と永続カウンタが成立することを起動時に確認する。"""
    required = {
...
def _notion_page_to_retry_candidate(page: dict) -> dict | None:
    ...
    return {
        ...
        "repo": {
            ...
            "sourceContext": "", "primaryUrl": page_url,
...
def get_pending_retry_items(limit: int = PENDING_RETRY_PER_RUN) -> list[dict] | None:
    if not NOTION_API_KEY or not NOTION_DATA_SOURCE_ID:
        return []
    headers = {"Authorization": f"Bearer {NOTION_API_KEY}", "Content-Type": "application/json", "Notion-Version": NOTION_API_VERSION}
    payload = {
    ...
    res = _query_notion_db_with_retry(f"https://api.notion.com/v1/data_sources/{NOTION_DATA_SOURCE_ID}/query", headers, payload)
    if res is None:
        logger.error("[PENDING RETRY] Notion再試行待ち取得に失敗。")
        return None
    items = [item for page in res.json().get("results", []) if (item := _notion_page_to_retry_candidate(page))]
...
def _mark_pending_retry_migration_completed(sha: str | None) -> bool:
    ...
    payload = {"message": "chore: complete pending retry legacy migration", "content": base64.b64encode((body + "\n").encode("utf-8")).decode("ascii"), "branch": os.environ.get("GEMINI_COUNTER_BRANCH", "main")}
    if sha:
        payload["sha"] = sha
    try:
        res = requests.put(f"https://api.github.com/repos/{repo}/contents/{PENDING_RETRY_MIGRATION_STATE_PATH.lstrip('/')}", headers=headers, json=payload, timeout=15)
...
def migrate_legacy_quality_failed_to_pending_retry() -> int:
    ...
    for page in pages:
        ...
        if candidate and update_notion_pending_retry(candidate["notion_page_id"], candidate["repo"]["nameWithOwner"], GROUNDING_METADATA_ONLY, [candidate["repo"]["url"]]):
            migrated += 1
...
def run_new_source_test_mode():
    ...
    try:
        ...
        logger.error(f"[NEW SOURCE TEST STOP] Screeningモデル利用不能: {e}")
