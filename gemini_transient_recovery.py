...
The caller already bounds retries per candidate and the production pipeline
bounds Deep Dive requests per run, so this layer adds no hot loop and no new
provider request path.  It only prevents repeated low-value recovery probes.
...
def install(pipeline_module):
    """Allow one later recovery probe after 503, then cool down the model."""
    ...
    def mark_model_unavailable(model_name: str, reason: str = "") -> None:
        if _is_transient_503(reason):
            ...
            if occurrence < _COOLDOWN_THRESHOLD:
                if logger is not None:
                    logger.warning(
                        "[MODEL TRANSIENT 503] %s occurrence=%s; fallback for this candidate, "
                        "one later recovery probe remains eligible",
                        model_name,
                        occurrence,
                    )
